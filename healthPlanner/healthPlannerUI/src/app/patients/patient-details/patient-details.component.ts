...
export class PatientDetailsComponent implements OnInit {
    ...
    this.service.getDataById(id).subscribe(
      response => {
        ...
        this.registerForm = new FormGroup({
          ...
          bmi: new FormControl(this.decimalPipe.transform(this.calcBMI(this.patient.height,this.patient.weight), '1.2-2')),
          category: new FormControl(this.getCategory(this.calcBMI(this.patient.height,this.patient.weight))),
        });
        this.registerForm.disable();
  ...
  calcBMI(hheight,wweight){
    var bmi=((wweight/Math.pow(hheight,2))*703),
        globalBMI=bmi;
  ...
  getCategory(bbmi){
    ...
    if(bbmi>30){
      category="Obese";
  } else if(bbmi>25 && bbmi<29.99) {
   category="Overweight";
  }else if(bbmi>18.5 && bbmi<24.99) {
   category="Healthy";
  }else if(bbmi<18.5) {
